...
export function activeApprovedReviewers(reviews: ReviewMetadata[]): string[] {
  const latestReviews = new Map<string, { review: ReviewMetadata; submittedAt: number | null; index: number }>();
  reviews.forEach((review, index) => {
    if (review.actorType !== undefined && review.actorType !== "human") return;

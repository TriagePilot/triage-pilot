...
In enforce mode, the stored selected individual reviewers form the policy cohort for `triagepilot/human-review-policy`. When TriagePilot first routes a pull request, approvals from individual GitHub users on that same head count toward the required cohort before it selects and requests additional reviewers. This preserves valid review work that predates TriagePilot. The check succeeds for a no-human route and after every selected user approves the current pull-request head. It remains in progress while an approval is missing or changes are requested, and it fails when a human-review route has no eligible individual reviewer or TriagePilot reaches a permanent evaluation failure. Team reviewer targets are invalid and result in no GitHub writes.

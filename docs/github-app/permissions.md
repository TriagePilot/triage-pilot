...
| Contents | Read-only | Read `.triagepilot.yml`, with `.github/triagepilot.yml` as a fallback, from the pull request's trusted base commit |
...
New and missing configurations use shadow mode. Shadow mode computes and stores decisions without writing to GitHub; only `mode: enforce` in the root `.triagepilot.yml` or legacy fallback `.github/triagepilot.yml` at the trusted pull-request base commit permits writes. When both paths exist, the root file wins. Configuration from an unmerged head commit is never used as write policy.

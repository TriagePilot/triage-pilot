...
In enforce mode, TriagePilot also creates `triagepilot/human-review-policy` on the routed head. It succeeds immediately for a no-human route, stays in progress while every selected individual has not approved the current head, and fails for an eligibility, configuration, authorization, or permanent evaluation failure. A human-review routing action creates a durable evaluation job immediately, and a `pull_request_review` webhook creates one for each later review event; the worker re-evaluates only the stored individual reviewer cohort using GitHub's current-head review state. The policy check is separate from the routing action outcome and is displayed separately in the dashboard.

...
- `apps/web` serves the administrator login and operations UI, receives GitHub webhooks, verifies signatures and organization scope, and inserts each webhook receipt and routing or human-review-policy job in one transaction.
- `apps/worker` claims jobs from PostgreSQL, creates GitHub App installation tokens, reads repository data, computes and stores routing decisions, applies actions only for explicit enforce configuration, evaluates required human-review policy checks and reviewer-absence activations, recovers interrupted jobs, updates its heartbeat, and removes expired data.
...
2. The web process accepts only routing-relevant pull-request actions from the configured organization. It records every accepted delivery ID with its action and hook ID, but creates at most one routing job for a repository, pull request, signed base SHA, and head SHA in a transaction. Administrator availability changes enqueue delayed `activate_reviewer_absence` jobs in that same PostgreSQL queue.
   An authenticated administrator can also recover missing or stalled routing from the operations ledger. The web process resolves the selected active repository, reads the pull request's current base SHA, head SHA, and draft state from GitHub, and enqueues a uniquely identified operator run through the same `process_pull_request` pipeline. Operator runs do not create synthetic webhook receipts.
...
PostgreSQL contains the organization installation, repositories, webhook receipts, jobs, routing decisions with action outcomes, and one current worker heartbeat. Reviewer availability adds three data groups: the singleton organization timezone setting, reviewer absence windows, and decision-scoped replacement history. Administrator and GitHub credentials remain in environment variables or mounted files and are not stored in PostgreSQL.

Reviewer availability uses delayed jobs in the existing queue; it adds no scheduler, separate service, or repository configuration parser.
